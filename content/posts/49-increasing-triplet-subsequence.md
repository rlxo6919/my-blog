...
이 제약이 이 문제의 진짜 주제입니다. 이 글에서는 `O(n^3)` 브루트포스부터 시작해서, 배열 두 개를 쓰는 `O(n)`/`O(n)` 풀이를 거쳐, 변수 두 개로 `O(n)`/`O(1)`에 푸는 **그리디 패턴**까지 단계적으로 내려가 보겠습니다. 시간·공간 복잡도 표기가 익숙하지 않다면 [시간 복잡도와 공간 복잡도 완전 정복](/posts/52-time-and-space-complexity)을 먼저 읽어 보시는 것을 권합니다.
