...
Hibernate 6은 JPQL의 기본 처리 방식으로 **`SELECT`** 를 선택합니다. `JOIN`으로 묶도록 바꾸려면 `@Fetch(FetchMode.JOIN)` 등 벤더 확장을 쓰거나, 명시적으로 `fetch join` 을 JPQL에 씁니다. 그래서 `EAGER`는 "알아서 효율적으로 묶어주겠지"가 아니라 **여전히 개별 쿼리**가 기본입니다.
...
`MultipleBagFetchException`이 발생합니다. JPA 명세가 **두 개 이상의 컬렉션 `JOIN FETCH`를 허용하지 않기 때문**입니다. 이유는 간단합니다 — 두 개의 `@OneToMany`를 동시에 조인하면 결과 행 수가 `부모 × 자식A × 자식B` 로 **카르테시안 곱**이 되어 폭발합니다. Hibernate는 이 상황을 미리 막습니다.
