...
> **참고:** 범위 조건이 사용된 컬럼 **이후의 컬럼**은 일반적인 B+Tree 탐색에서 스캔 범위를 더 좁히기 어려워집니다. `(a, b, c)` 인덱스에서 `WHERE a = 1 AND b > 10 AND c = 100`이라면, 보통 `a`와 `b`까지만 탐색 범위를 줄이고 `c`는 추가 필터링으로 활용될 가능성이 큽니다.
