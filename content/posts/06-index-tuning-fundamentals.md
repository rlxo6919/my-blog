...
대부분의 RDBMS(MySQL, PostgreSQL, Oracle)는 기본 인덱스로 **B-Tree(Balanced Tree)** 를 사용합니다. B-Tree가 선택된 이유는 명확합니다.

- **항상 균형을 유지** — 어떤 데이터를 찾든 트리의 높이가 일정합니다
- **범위 검색에 유리** — 리프 노드가 정렬된 상태로 연결되어 있어 `BETWEEN`, `>`, `<` 같은 범위 쿼리에 효율적입니다
...
- **리프 노드(Leaf Node)** — 실제 인덱스 키 값과 데이터의 위치(포인터)가 저장됩니다. 리프 노드끼리 **양방향 링크드 리스트**로 연결되어 있어 정렬 순서대로 순회할 수 있습니다
...
트리의 높이가 3이라면, **단 3번의 비교**로 원하는 데이터를 찾을 수 있습니다. 100만 행이든 1,000만 행이든 B-Tree의 높이는 보통 3~4 수준이므로, 데이터 양이 늘어도 검색 속도가 크게 변하지 않습니다.

> **참고:** B-Tree의 시간 복잡도는 **O(log N)** 입니다. 1,000만 행이라도 약 23번의 비교(log₂ 10,000,000 ≈ 23)면 충분합니다. 풀 테이블 스캔의 O(N)과는 비교할 수 없는 차이입니다.
...
-- ❌ 인덱스 사용 불가 (첫 번째 컬럼을 건너뜀)
WHERE age = 28
```

`age`만으로 검색하면 인덱스를 사용할 수 없습니다. `name`이 정해지지 않은 상태에서 `age`는 정렬되어 있지 않기 때문입니다. 이것을 **최좌선 접두사 규칙(Leftmost Prefix Rule)** 이라고 합니다.
...
> **참고:** 범위 조건이 사용된 컬럼 **이후의 컬럼**은 인덱스를 활용하지 못합니다. `(a, b, c)` 인덱스에서 `WHERE a = 1 AND b > 10 AND c = 100`이라면, `a`와 `b`까지만 인덱스를 타고 `c`는 필터링으로 처리됩니다.
...
인덱스로 조회할 행이 전체 데이터의 약 **20~25% 이상**이면, 옵티마이저는 인덱스를 거치는 것보다 풀 테이블 스캔이 더 효율적이라고 판단합니다. 보조 인덱스의 랜덤 I/O 비용이 순차 스캔보다 커지기 때문입니다.
...
5. **EXPLAIN** — 쿼리 튜닝의 시작이자 끝입니다. 인덱스를 만들었으면 반드시 실행 계획으로 확인합니다
