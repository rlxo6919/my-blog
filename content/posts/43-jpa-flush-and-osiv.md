...
3. action queue 정렬 — INSERT → UPDATE → DELETE 순서로 SQL 생성
4. JDBC 배치로 SQL 전송
...
같은 엔티티에 대해 `UPDATE`와 `DELETE`를 섞어 부르면 Hibernate가 재정렬합니다. 그래서 **메서드에서 호출한 순서와 실제 SQL이 나가는 순서는 다를 수 있습니다**. 고아 객체 제거(`orphanRemoval`)가 예상과 다르게 동작하는 이유의 상당수가 이 재정렬 규칙 때문입니다.
