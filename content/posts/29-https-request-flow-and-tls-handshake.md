...
```text
Client -> Server : ClientHello                        (평문)
Client <- Server : ServerHello                        (평문)
--- 이 시점부터 handshake key로 암호화 ---
...
`TLS 1.3`에서 가장 큰 변화 중 하나는, `ClientHello`/`ServerHello`를 통해 키 교환이 끝나는 순간부터 **`EncryptedExtensions`, `Certificate`, `CertificateVerify`, `Finished`가 모두 handshake 트래픽 키로 암호화되어 전송된다**는 점입니다(`RFC 8446 §4`). 서버 인증서조차 평문으로 노출되지 않기 때문에, `TLS 1.2` 시절의 평문 핸드셰이크 다이어그램과는 흐름이 다릅니다. 위 다이어그램의 중괄호 `{ ... }`는 해당 메시지가 암호화된 상태임을 표시한 것입니다.
