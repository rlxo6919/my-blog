...
이 문제의 입력에는 영문자, 공백, 지정된 문장 부호만 나오므로 위 방식도 통과할 수 있습니다. 다만 정규식에서 `\W`는 "알파벳이 아닌 문자"가 아니라 "단어 문자가 아닌 문자"에 가깝습니다. 즉 숫자나 `_`는 단어 문자로 취급됩니다. 이 문제에서는 숫자와 `_`가 나오지 않지만, 문제 조건을 더 직접적으로 표현하려면 `Regex("[^a-z]+")`처럼 소문자 알파벳이 아닌 문자를 구분자로 보는 편이 더 명확합니다.
