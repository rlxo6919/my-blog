...
### 문제: innerJoin이 데이터를 누락시키고, 참여자 로드가 비효율적이다

채팅방 목록을 조회할 때 두 가지 성능 이슈가 있었습니다.

1. **Join 타입 문제**: `innerJoin`으로 인해 참여자가 없는 채팅방이 결과에서 누락
2. **연관 데이터 로드 방식**: 채팅방 목록을 가져온 뒤, 참여자 정보를 별도 변수에 저장하면서 영속성 컨텍스트를 활용하지 못함

```kotlin
// Before: innerJoin + 결과를 별도 변수로 관리
val rooms = queryFactory
    .selectFrom(room)
    .where(/* 조건 */)
    .fetch()

// 참여자를 별도 쿼리로 로드하지만, 결과를 사용하지 않고 버림
if (rooms.isNotEmpty()) {
    queryFactory
        .selectFrom(room)
...
| `.also {}` 블록으로 반환 흐름 정리 | 첫 번째 쿼리 결과를 그대로 반환하면서, 영속성 컨텍스트에 연관 엔티티를 미리 로드 |
