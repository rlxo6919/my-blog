...
### 문제: `innerJoin`이 데이터를 누락시키고, 반환 흐름이 불명확하다
...
1. **JPA 영속성 컨텍스트를 활용한 배치 페칭은 `fetchJoin`과 `.also` 블록으로 구현할 수 있습니다.** — 같은 트랜잭션 내에서 1차 캐시에 올라간 엔티티의 연관 관계를 채워주는 방식입니다.
...
4. **직렬화 매핑은 가능하면 API 경계에 두는 편이 좋습니다.** — Service가 Protobuf 같은 직렬화 프로토콜에 직접 의존하지 않도록 분리하면 변경 영향 범위를 줄일 수 있습니다.
